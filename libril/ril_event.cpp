...
{
    dlog("~~~~ Event %lx ~~~~", (unsigned long)ev);
    dlog("     next    = %lx", (unsigned long)ev->next);
    dlog("     prev    = %lx", (unsigned long)ev->prev);
    dlog("     fd      = %d", ev->fd);
    dlog("     pers    = %d", ev->persist);
    dlog("     timeout = %ds + %dus", (int)ev->timeout.tv_sec, (int)ev->timeout.tv_usec);
    dlog("     func    = %lx", (unsigned long)ev->func);
    dlog("     param   = %lx", (unsigned long)ev->param);
    dlog("~~~~~~~~~~~~~~~~~~");
}
#else
...
{
    dlog("~~~~ ril_event_set %lx ~~~~", (unsigned long)ev);
