...
void convertRilDataCallToHal(RIL_Data_Call_Response_BB *dcResponse,
        SetupDataCallResult& dcResult);
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_STOP_NETWORK_SCAN);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
}
 ...
 * @param dataProfiles the pointer to the actual data profiles. The acceptable type is RIL_DataProfileInfo.
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_START_LCE);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
}
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_STOP_LCE);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
}
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_PULL_LCEDATA);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
}
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_GET_ACTIVITY_INFO);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
}
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_GET_CARRIER_RESTRICTIONS);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
}
...
Return<void> RadioImpl::sendDeviceState(int32_t serial, DeviceStateType deviceStateType,
    ...
    if (deviceStateType ==  DeviceStateType::LOW_DATA_EXPECTED) {
	    ...
	    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_SEND_DEVICE_STATE);
	    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
	}
    return Void();
}
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_SET_UNSOLICITED_RESPONSE_FILTER);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
}
...
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_SET_SIM_CARD_POWER);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
}
...
#if VDBG
    RLOGD("setSimCardPower_1_1: serial %d", serial);
#endif
    RequestInfo *pRI = android::addRequestToList(serial, mSlotId, RIL_REQUEST_SET_SIM_CARD_POWER);
    sendErrorResponse(pRI, RIL_E_REQUEST_NOT_SUPPORTED);
    return Void();
...
int radio::getIccCardStatusResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (e == RIL_E_SUCCESS && responseLen == 0) {
        ...
        } else if (response == NULL || responseLen != sizeof(RIL_CardStatus_v6)
            ...
            RLOGE("getIccCardStatusResponse: Invalid response %d", responseLen);
            if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
        } else {
        ...
        if (response == NULL) {
            ...
            if (numStrings != 15) {
				...
				for (int i = 0; i < numStrings; i++)
                	RLOGD("getVoiceRegistrationStateResponse: resp[%d]: %s", i, resp[i]);
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (response == NULL) {
            ...
            if ((numStrings != 6) && (numStrings != 11) && (numStrings != 12) && (numStrings != 13)) {
				...
				for (int i = 0; i < numStrings; i++)
                	RLOGD("getDataRegistrationStateResponse: resp[%d]: %s", i, resp[i]);
...
int radio::setupDataCallResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (response == NULL || (responseLen % sizeof(RIL_Data_Call_Response_BB) != 0
                && responseLen % sizeof(RIL_Data_Call_Response_v9) != 0
                && responseLen % sizeof(RIL_Data_Call_Response_v6) != 0)) {
            if (response != NULL) {
                RLOGE("setupDataCallResponse: Invalid response, responseLen: %x", responseLen);
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            }
        ...
        } else if ((responseLen % sizeof(RIL_Data_Call_Response_BB)) == 0) {
            convertRilDataCallToHal((RIL_Data_Call_Response_BB *) response, result);
...
int radio::getDataCallListResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if ((response == NULL && responseLen != 0)
                || (responseLen % sizeof(RIL_Data_Call_Response_BB) != 0
                && responseLen % sizeof(RIL_Data_Call_Response_v9) != 0
                && responseLen % sizeof(RIL_Data_Call_Response_v6) != 0)) {
...
int radio::getCdmaSubscriptionSourceResponse(int slotId,
    ...
    RLOGE("getCdmaSubscriptionSourceResponse: responseLen: %x", responseLen);
...
int radio::getRadioCapabilityResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        populateResponseInfo(responseInfo, serial, responseType, e);
        result.logicalModemUuid = hidl_string();
...
void convertRilDataCallListToHal(void *response, size_t responseLen,
    ...
    if ((responseLen % sizeof(RIL_Data_Call_Response_BB)) == 0) {
        num = responseLen / sizeof(RIL_Data_Call_Response_BB);
        RIL_Data_Call_Response_BB *dcResponse = (RIL_Data_Call_Response_BB *) response;
...
int radio::dataCallListChangedInd(int slotId,
    ...
    if (radioService[slotId] != NULL && radioService[slotId]->mRadioIndication != NULL) {
        if ((response == NULL && responseLen != 0)
                ...
                && responseLen % sizeof(RIL_Data_Call_Response_v9) != 0
                && responseLen % sizeof(RIL_Data_Call_Response_v6) != 0)) {
            RLOGE("dataCallListChangedInd: invalid response, responseLen: %d", responseLen);
            RLOGE("dataCallListChangedInd: sizeof(RIL_Data_Call_Response_BB): %d", sizeof(RIL_Data_Call_Response_BB));
            return 0;
        }
